...
$url = "http://www.imdb.com/search/title?view=simple&count=5&title_type=game&genres=";
$cats = array(
	"action",
	"adventure",
	"animation",
		...
		foreach($ids as $id) {
			...
			if($json = file_get_contents("http://imdbapi.org/?type=json&episode=0&id=".urlencode($id))) {
				...
				if (!isset($data->{'code'},$data->{'error'})) {
					...
					mysql_query("INSERT INTO `db_list` (`name`,`imdb_id`,`year`,`rating`,`category`) VALUES ('{$title}','{$id}','{$year}','{$rating}','3');");
